...
run_parameter_parser:
	...
	cd ${PARSER_DIR} &&	pip install -r requirements.txt && python parameter_parser.py
...
run_outputs_parser:
	echo "Generating v2 parameter config file"
	cd ${PARSER_DIR} &&	pip install -r requirements.txt && python parameter_parser.py --type outputs --output-file outputs_config.yaml
