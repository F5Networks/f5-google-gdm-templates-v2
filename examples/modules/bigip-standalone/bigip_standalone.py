    ...
    metadata_config = {
                'items': [
                {
                    ...
                    'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    '    nohup /config/mgmt-route-script.sh &',
                                    '    exit',
                                    'fi',
                                    'if [ -f /config/first_run_flag ]; then',
                                    '   echo "$(date) Skip first run steps, already ran."',
                                    'else',
                                    ...
                                    '   exec 1>$npipe',
                                    '   exec 2>&1',
                                    '   echo "$(date) Running Nic Swap Script"',
                                    ...
                                    '   bash /var/config/rest/downloads/f5-bigip-runtime-init.gz.run -- \'--cloud gcp --telemetry-params templateName:v3.5.0.0/examples/modules/bigip-standalone/bigip_standalone.py\'',
                                    '   /usr/bin/cat /config/cloud/runtime-init-conf.yaml',
                                    '   echo "Restarting Services"',
                                    '   bigstart restart; echo "Sleep for 60 seconds"; sleep 60',
                                    '   echo "Done, running bigip runtime init."',
                                    ...
                                    'EOF',
                                    '   # Workaround script for BZ 2186789, mgmt rt lost after reboot',
                                    '   /usr/bin/cat << \'EOF\' > /config/mgmt-route-script.sh',
                                    '   #!/bin/bash',
                                    '   LOG_FILE=/var/log/cloud/mgmt-config-after-reboot.log',
                                    '   /usr/bin/touch $LOG_FILE',
                                    '   npipe=/tmp/$$.tmp',
                                    '   /usr/bin/trap "rm -f $npipe" EXIT',
                                    '   /usr/bin/mknod $npipe p',
                                    ...
                                    '   exec 1>$npipe',
                                    '   exec 2>&1',
                                    '   echo "$(date) Running Mgmt Route Script"',
                                    '   MULTI_NIC=' + str(multi_nic),
                                    '   if [[ ${MULTI_NIC} == "True" ]]; then',
                                    '       # Need to remove existing and recreate a MGMT default route as not provided by DHCP on 2nd NIC Route name must be same as in DO config.',
                                    '       source /usr/lib/bigstart/bigip-ready-functions',
                                    ...
                                    '       echo "MGMT_NETWORK - "',
                                    '       echo $MGMT_NETWORK',
                                    '       echo "tmsh list sys management-ip before create - "',
                                    '       tmsh list sys management-ip',
                                    '       echo "Current Linux route table -"',
                                    '       route -n',
                                    '       tmsh create sys management-ip ${SELF_IP_MGMT}/32',
                                    '       echo "tmsh list sys management-ip afer create - "',
                                    '       tmsh list sys management-ip',
                                    '       echo "Current Linux route table -"',
                                    '       tmsh create sys management-route mgmt_gw network ${MGMT_GW}/32 type interface',
                                    '       tmsh create sys management-route mgmt_net network ${MGMT_NETWORK}/${MGMT_BITMASK} gateway $MGMT_GW',
                                    ...
                                    '   /usr/bin/chmod +x /config/mgmt-route-script.sh',
                                    '   MULTI_NIC=' + str(multi_nic),
